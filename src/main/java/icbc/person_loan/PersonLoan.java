package icbc.person_loan;

import icbc.Icbc;
import icbc.interaction.Goods;
import icbc.interaction.PersonalLoanGoods;
...
public class PersonLoan extends BaseThread {
    ...
    protected void workImps() throws Exception {
        ...
        writeFile( objTansJson(personLoanBean));
    }

    @Override
    protected void work2Imps() throws Exception {
        ArrayList<PersonalLoanGoods> goodsList = new ArrayList<>();
        String hUrl = "http://www.icbc.com.cn";  //  http://www.icbc.com.cn/HtmlPatch/ICBC/sy/gerendaikuan.htm
        ...
        while (iterator.hasNext()){
            ...
            goods.setId((i++)+"");
            val = element.select("img").first().attr("src");
            if (!val.contains(hUrl)){
                val = hUrl+val;
            }
            if (val.indexOf(";")>0){
            ...
            goods.toGoodsRecode();
            val = element.select("a").first().attr("href");
            if (!val.startsWith(hUrl)){
                val = hUrl+val;
            }
            goods.setLink(val);
            goodsList.add(goods);
