package offline_xperience_achine;

import interfaces.ActionCall;
import interfaces.BaseThread;

import lunch.Param;
import offline_xperience_achine.obj.GroupsSeckillsBean;
import offline_xperience_achine.obj.groups.Groups;
...
public class GroupBuy extends BaseThread {
    ...
    public void setSeckillsItems(Groups groups, String url) {
        try {
            ...
            groups.setPrice(doc.select("span.spancolor_gre").first().text().replace("гд", ""));
...
//            e.printStackTrace();
    ...
    public String getGoodsImageUrl(String url, String path) {
        return super.getGoodsImageUrl(url, path).substring(1);
    }
}
